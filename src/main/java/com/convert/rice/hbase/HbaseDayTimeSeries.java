...
public class HbaseDayTimeSeries implements TimeSeries {
    ...
    public static final DateTimeFormatter FORMATER = DateTimeFormat.forPattern("yyyyMMdd").withZoneUTC();;
     ...
     * @param hTable
     */
    public HbaseDayTimeSeries(HTablePool pool) {
        this.pool = pool;
    }
    ...
    public Collection<DataPoints> get(String type, String key, Interval interval, Collection<String> metrics)
        ...
        DateTime end = interval.getEnd().plusDays(1); // we add one day since the scan is exclusive.
        String startRow = getRowKey(key, start);
        String endRow = getRowKey(key, end);
        Scan scan = new Scan(Bytes.toBytes(startRow), Bytes.toBytes(endRow));
        scan.addFamily(DEFAULT_FAMILY);
        scan.setCaching(Math.min(THIRTY_DAYS, period.getHours()));
        ...
        for (Result result : results) {
            ...
            DateTime day = FORMATER.parseDateTime(split[split.length - 1]);
